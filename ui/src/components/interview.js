...
const Interview = () => {
        ...
        async function fetchData(page, pageSize, pageType) {
            try {
                ...
                if (searchTerm || selectedLanguages.length > 0 || selectedButtons.length > 0) {
                    ...
                    response = await fetch(`http://localhost:3033/knowledge/getPaginatedKnowledge?page=${page}&pageSize=${pageSize}&pageType=Interview`);
    ...
    const handleSubmit = (e) => {
        ...
        // if (searchTerm) {
        //     // if (language === 'all') {
        //     //     results = knowledgeList.filter(item => item.title.includes(searchTerm));
        //     // } else {
        //     //     results = knowledgeList.filter(item =>
        //     //         item.title.includes(searchTerm) && item.title.includes(language)
        //     //     );
        //     // }

        //     // 二次过滤基于 selectedButtons 的标签
        //     if (selectedButtons.length > 0) {
        //         results = results.filter(item =>
        //             item.tags.some(tag => selectedButtons.includes(Number(tag))) // 将tag转换为数字然后进行比较
        //         );
        //     }
        // } else {
            ...
            <div className="container mt-4 dashboard-content">
                ...
                <ul className="list-group">
                    {displayedKnowledge.map(item => (
                        <li key={item.guid} className="list-group-item">
