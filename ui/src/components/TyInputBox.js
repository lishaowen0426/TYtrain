  ...
  return (
    <div
      className={`form-group ${
        isHorizontal ? "d-flex align-items-center" : ""
      }`}
    >
      {label && (
        <label
          htmlFor={id}
          className={`font-ty text-1xl ${isHorizontal ? "me-2" : "mb-1"}`}
        >
          {label}
        </label>
      )}
      <input
        type="text"
        id={id}
        className={`form-control ty-checkbox font-ty ${
          error ? "is-invalid" : ""
        } ${hasText ? "bg-sky-000" : "bg-sky-100"} ${
